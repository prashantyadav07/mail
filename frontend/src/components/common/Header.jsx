...
const Header = () => {
  ...
  return (
    <header className="bg-white shadow-md">
      {/* --- RESPONSIVE CHANGE: Flex direction and alignment updated --- */}
      <nav className="container mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:justify-between sm:items-center">
        <div className="flex justify-between items-center">
          ...
          {/* Add a menu button here for mobile if you have many links */}
        </div>
        
        {/* --- RESPONSIVE CHANGE: Margin and spacing adjusted for mobile --- */}
        <div className="flex items-center justify-center sm:justify-end space-x-4 mt-4 sm:mt-0">
          {user ? (
            <>
              ...
              {user.role === 'admin' ? (
                 <Link to="/admin/dashboard" className="px-3 py-2 text-sm font-medium text-gray-700 rounded-md hover:bg-gray-100">
                  Admin
                </Link>
              ) : (
                <Link to="/user/dashboard" className="px-3 py-2 text-sm font-medium text-gray-700 rounded-md hover:bg-gray-100">
                  Dashboard
                </Link>
              )}
              <button
